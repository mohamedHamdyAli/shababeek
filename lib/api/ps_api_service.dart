...
class PsApiService extends PsApi {
  ...
  Future<PsResource<List<Product>>> getAllItems(
      {required String adsStatus}) async {
    final String url =
        '${PsUrl.ps_item_detail_url}/api_key/${PsConfig.ps_api_key}/ads_status/$adsStatus/status/1';
    // return await getServerCall<Product, Product>(Product(), url);
    return await getServerCall<Product, List<Product>>(Product(), url);
  }

  Future<PsResource<List<Package>>> getAllpackages() async {
    final String url =
        '${PsUrl.ps_get_packages}/api_key/${PsConfig.ps_api_key}';
    // return await getServerCall<Product, Product>(Product(), url);
